...
GEO_Q_CSV = "/Dual-Policy Reinforcement Learning Gating Architecture for Bio-Inspired Autonomous Navigation Agent/Geocentric_policy_framework/angles_to_goal.csv"
EGO_VECTOR_TABLE_CSV = "/Dual-Policy Reinforcement Learning Gating Architecture for Bio-Inspired Autonomous Navigation Agent/Egocentric_policy_framework/ego_vector_table.csv"

OUTPUT_DIR = "/Dual-Policy Reinforcement Learning Gating Architecture for Bio-Inspired Autonomous Navigation Agent/Gating_Policy_Network/final_analysis_run"
os.makedirs(OUTPUT_DIR, exist_ok=True)
MODEL_SAVE_PATH = os.path.join(OUTPUT_DIR, 'gating_model_pytorch.pth')

# Maze & start/goal configuration
MAZE_SIZE = (16, 16)
START_POS = (1, 1)
GOAL_POS  = (14, 14)
HAZARD_DENSITY_TRAINING = 0.20
...
# --- 5. The DQN Agent (PyTorch Implementation) ---
class DQN(nn.Module):
...
# --- 6. Environment Simulation Functions ---
...
def plot_learning_curves(rewards, output_dir):
    # <<< MODIFIED: This function now only plots the reward curve >>>
    ...
    plt.savefig(filepath); plt.show(); print(f"Reward learning curve saved to {filepath}")
...
def plot_loss_curve(losses, output_dir, window=100):
    ...
    plt.savefig(filepath); plt.show(); print(f"Loss curve saved to {filepath}")
...
def plot_trajectory(maze, path, goal_pos, output_dir, filename='trajectory_unseen_test.png'):
    ...
    filepath = os.path.join(output_dir, filename); plt.savefig(filepath); plt.show(); print(f"Trajectory plot saved to {filepath}")
...
def plot_policy_map(agent, maze, ego_data, goal_pos, output_dir, filename='policy_map_unseen_test.png'):
    ...
    filepath = os.path.join(output_dir, filename); plt.savefig(filepath, bbox_inches='tight'); plt.show(); print(f"Policy map saved to {filepath}")
...
def analyze_performance_vs_density(agent, ego_vector_table, output_dir):
    ...
    for density in hazard_densities:
        print(f"--- Testing hazard density: {density*100:.0f}% ({mazes_per_density} mazes) ---")
        ...
        for i in range(mazes_per_density):
            print(f"  - Simulating maze {i+1}/{mazes_per_density}...", end='\r')
        ...
        print(f"\n  - Completed analysis for {density*100:.0f}% density.")
...
def plot_performance_vs_density(results, output_dir):
    ...
    filepath = os.path.join(output_dir, 'performance_vs_density.png'); plt.savefig(filepath); plt.show(); print(f"Performance vs density plot saved to {filepath}")
...
def plot_policy_distribution_charts(results, output_dir):
    ...
    plt.savefig(filepath_bar); plt.show(); print(f"Policy distribution bar chart saved to {filepath_bar}")
    ...
    plt.savefig(filepath_curve); plt.show(); print(f"Policy distribution curve plot saved to {filepath_curve}")
...
def plot_all_training_policy_maps(agent, mazes, ego_maps, goal_pos, output_dir):
    ...
    # Save current epsilon to restore later, set to 0 for pure exploitation during plotting
    ...
    for idx, (maze, ego_data) in enumerate(zip(mazes, ego_maps)):
        ...
        # Plot maze background (1=black/hazard, 0=white/free)
        ax.imshow(maze, cmap='Greys', interpolation='none')

        # Iterate over every cell in the maze
        for r in range(maze.shape[0]):
            for c in range(maze.shape[1]):
                # Skip hazards and the goal itself (no action needed at goal)
                if maze[r, c] == 0 and (r, c) != goal_pos:
                    
                    # 1. Get State
                    state = agent.get_state((r, c), maze, ego_data)
                    
                    # 2. Choose Action (Greedy)
                    with torch.no_grad():
                        action = agent.policy_net(state).argmax().item()

                    # 3. Determine intended next position
                    if action == 0:
                        tentative_next_pos = get_next_pos_from_angle_STANDARD(r, c, agent.geo_data[(r, c)], maze)
                        policy = 'Geocentric'
                    else:
                        tentative_next_pos = get_next_pos_from_vector(r, c, ego_data[(r, c)], maze)
                        policy = 'Egocentric'

                    # 4. Check for Override (Stuck condition)
                    # If the chosen action leads to a wall or stays in place, the agent relies on the 'is_stuck' logic
                    is_stuck = (tentative_next_pos == (r, c))
                    
                    if is_stuck:
                        policy = 'Override'
                        # Find the best valid neighbor (simple lookahead for plotting)
                        valid_moves = []
                        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                            nr, nc = r + dr, c + dc
                        ...
                        # Logic matches the simulation's stuck handler: pick neighbor closest to goal
                        if valid_moves:
                        ...
                        dr = tentative_next_pos[0] - r
                        dc = tentative_next_pos[1] - c

                    # 5. Plot the Arrow
                    if not (dr == 0 and dc == 0):
                        ax.arrow(c, r, dc * 0.4, dr * 0.4, 
                                 head_width=0.3, head_length=0.3, 
                                 fc=policy_colors[policy], ec=policy_colors[policy])

        # Add Markers for Start and Goal
        ax.plot(START_POS[1], START_POS[0], 'bs', ms=12, label='Start') # Blue Square
        ax.plot(goal_pos[1], goal_pos[0], 'g*', ms=18, label='Goal')    # Green Star
...
if __name__ == "__main__":
    ...
    print(f"\nGenerating {NUM_TRAINING_MAZES} fixed mazes for training...")
    training_mazes = [generate_maze(MAZE_SIZE, HAZARD_DENSITY_TRAINING, START_POS, GOAL_POS) for _ in range(NUM_TRAINING_MAZES)]
    training_ego_maps = [map_vectors_to_maze(m, ego_vector_table) for m in training_mazes]
    print("Training maze set generated successfully.")
    ...
    print("\n" + "="*45 + "\n--- Starting Training on Fixed Maze Set (PyTorch) ---\n" + "="*45)
    for e in range(EPISODES):
        ...
        if (e+1) % TARGET_UPDATE_FREQUENCY == 0: agent.update_target_model(); print(f"--- Target network updated at episode {e+1} ---")
        agent.decay_epsilon(); goal_msg = "   *** GOAL REACHED! ***" if next_pos == GOAL_POS else ""
        print(f"Ep: {e+1}/{EPISODES} (Maze {maze_index+1}), Steps: {step+1}, Reward: {total_reward:.2f}, Epsilon: {agent.epsilon:.2f}{goal_msg}")

    print("\n" + "="*45 + "\n--- Training Complete, Generating Final Analysis... ---\n" + "="*45)
    agent.save_model(MODEL_SAVE_PATH)
    
    # 1. Plot standard learning and loss curves
    plot_learning_curves(rewards_per_episode, OUTPUT_DIR)
    plot_loss_curve(training_losses, OUTPUT_DIR)

    # --- NEW ADDITION HERE ---
    # Plot Policy Maps for all 10 Training Mazes
    plot_all_training_policy_maps(agent, training_mazes, training_ego_maps, GOAL_POS, OUTPUT_DIR)
    # -------------------------
    
    # 2. Find and plot one successful UNSEEN test maze trajectory and policy map
    print("\n--- Running evaluation to get a sample trajectory/policy map for one UNSEEN maze ---")
    final_maze, final_path, final_ego = None, None, None
    num_eval_mazes = 100
    eval_density = 0.20
    agent.epsilon = 0
    for i in range(num_eval_mazes):
        test_maze = generate_maze(MAZE_SIZE, eval_density, START_POS, GOAL_POS)
        ...
        if goal_reached:
             final_maze, final_path, final_ego = test_maze, path, test_ego_data
             print(f"Found a successful path on test maze {i+1} for visualization.")
             break
    
    if final_maze is not None:
        plot_trajectory(final_maze, final_path, GOAL_POS, OUTPUT_DIR)
        plot_policy_map(agent, final_maze, final_ego, GOAL_POS, OUTPUT_DIR)
    else:
        print("\nCould not find a successful path in the sample evaluation to visualize.")

    # 3. Run the full, slow analysis as the final step
    analyze_performance_vs_density(agent, ego_vector_table, OUTPUT_DIR)